...
class DataPreprocessingToolkit(object):
    ...
    def add_night_price(self, df):
        ...
        #  ---start---
        # average accomodation price per night per room
        average_accomodation = df["accomodation_price"] / df["length_of_stay"] / df["n_rooms"]
        df.loc[:, "night_price"] = np.round(average_accomodation, 2)
        #  ---end---
        return df
    ...
    def map_night_price_to_room_segment_buckets(self, df):
        # Write your code here
        # You must calculate average of night prices for every room_group_id and map those prices to buckets
        # (you can apply the map_value_to_bucket method which is available in the data_preprocessing_toolkit,
        # the buckets are available under self.room_segment_buckets). The new column should be named 'room_segment'.
